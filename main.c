...
Error extract_data(XImage *img, unsigned char **data_out) {
    ...
    mask = img->red_mask;   while ((mask & 1) == 0) { mask >>= 1; r_shift++; }
    mask = img->green_mask; while ((mask & 1) == 0) { mask >>= 1; g_shift++; }
    mask = img->blue_mask;  while ((mask & 1) == 0) { mask >>= 1; b_shift++; }
...
FilenameWithError make_filename(void) {
    ...
    int len = snprintf(NULL, 0,
                       "%04d-%02d-%02d_%02d-%02d-%02d.screenshot.png",
...
Error save_png(const char *filename, unsigned char *data, int width, int height) {
